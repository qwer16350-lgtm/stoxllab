...
def test_phase41b_45a_safe_prep_summary() -> None:
    ...
    assert_true(report["phase41b_private_test_reply_one_shot"]["actual_runtime_path_available"] is True, "41B runtime path")
    assert_true(report["phase41b_private_test_reply_one_shot"]["actual_runtime_executed"] is False, "41B no runtime")
    assert_true(report["phase41b_private_test_reply_one_shot"]["real_discord_send_adapter_wired"] is True, "41B real adapter wired")
    assert_true(report["phase41b_private_test_reply_one_shot"]["fake_adapter_contract_passed"] is True, "41B fake contract")
    assert_true(report["phase41b_private_test_reply_one_shot"]["message_sent_count"] == 0, "41B no send")
    assert_true(report["phase41c_actual_reply_closeout"]["actual_private_test_reply_verified"] is True, "41C success observed")
    assert_true(report["phase41c_actual_reply_closeout"]["message_sent_count"] == 1, "41C observed count 1")
    assert_true(report["phase41c_actual_reply_closeout"]["sent_scope"] == "private_test_only", "41C private scope")
    assert_true(report["phase41c_actual_reply_closeout"]["repeat_send_blocked"] is True, "41C repeat locked")
    assert_true(report["phase41c_actual_reply_closeout"]["ready_for_repeat_send"] is False, "41C no repeat")
    assert_true(report["phase41c_actual_reply_closeout"]["ready_for_supervised_session"] is False, "41C no direct session")
    assert_true(report["phase41c_actual_reply_closeout"]["discord_api_send_called_during_phase41c"] is False, "41C no API")
    assert_true(report["phase41c_actual_reply_closeout"]["discord_message_sent_during_phase41c"] is False, "41C no message")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["default_blocked"] is True, "42 default blocked")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["actual_runtime_cli_available"] is True, "42 actual CLI")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["allow_flag_available"] is True, "42 allow flag")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["runtime_default_blocked"] is True, "42 runtime default blocked")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["runtime_allow_flag_present"] is False, "42 allow absent")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["runtime_message_sent_count"] == 0, "42 runtime no send")
    assert_true(report["phase42_actual_session_closeout"]["available"] is True, "42 closeout")
    assert_true(report["phase42_actual_session_closeout"]["phase42_actual_supervised_private_test_session_succeeded"] is True, "42 success")
    assert_true(report["phase42_actual_session_closeout"]["phase42_exactly_once_supervised_success_recorded"] is True, "42 exactly once")
    assert_true(report["phase42_actual_session_closeout"]["message_sent_count"] == 1, "42 observed one send")
    assert_true(report["phase42_actual_session_closeout"]["sent_scope"] == "private_test_only", "42 private-test")
    assert_true(report["phase42_actual_session_closeout"]["session_lock_consumed"] is True, "42 lock consumed")
    assert_true(report["phase42_actual_session_closeout"]["phase42_repeat_supervised_session_locked"] is True, "42 repeat locked")
    assert_true(report["phase42_actual_session_closeout"]["phase42_repeat_supervised_session_allowed"] is False, "42 repeat disallowed")
    assert_true(report["phase42_actual_session_closeout"]["discord_api_send_called_during_closeout"] is False, "42 closeout no API")
    assert_true(report["phase42_actual_session_closeout"]["discord_message_sent_during_closeout"] is False, "42 closeout no message")
    assert_true(report["phase42_actual_session_closeout"]["additional_message_sent_count_during_closeout"] == 0, "42 closeout no extra send")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["blocked"] is True, "42 blocked")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["manual_gate_required"] is True, "42 manual gate")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["manual_gate_open"] is False, "42 gate closed")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["actual_supervised_session_executed"] is False, "42 no supervised session")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["actual_runtime_executed"] is False, "42 no runtime")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["discord_api_send_called"] is False, "42 no API")
    assert_true(report["phase42_supervised_private_test_session_preflight"]["ready_for_phase41b_repeat_send"] is False, "42 no repeat")
    assert_true(report["phase43_routing_rate_limit_policy"]["public_team_blocked"] is True, "43 public/team blocked")
    assert_true(report["phase43_routing_rate_limit_policy"]["phase41b_repeat_send_locked"] is True, "43 41B repeat locked")
    assert_true(report["phase43_routing_rate_limit_policy"]["phase42_repeat_supervised_session_locked"] is True, "43 42 repeat locked")
    assert_true(report["phase43_routing_rate_limit_policy"]["phase42_manual_gate_open"] is False, "43 42 gate closed")
    assert_true(report["phase43_routing_rate_limit_policy"]["ready_for_phase42_actual_supervised_session"] is False, "43 no 42 actual")
    ...
    assert_true(report["phase45_actual_llm_call_closeout"]["phase45_actual_llm_one_shot_completed"] is True, "45 closeout completed")
    assert_true(report["phase45_actual_llm_call_closeout"]["phase45_actual_llm_call_count"] == 1, "45 historical count")
    assert_true(report["phase45_actual_llm_call_closeout"]["phase45_ready_for_repeat_llm_call"] is False, "45 no repeat")
    assert_true(report["phase45_actual_llm_call_closeout"]["output_safety_blocked"] is True, "45 output blocked")
    assert_true(report["phase45_actual_llm_call_closeout"]["discord_message_sent"] is False, "45 no Discord")
    assert_true(report["phase46_blocked_llm_output_review"]["metadata_only"] is True, "46 metadata")
    assert_true(report["phase46_blocked_llm_output_review"]["raw_output_included"] is False, "46 no raw")
    assert_true(report["phase46_blocked_llm_output_review"]["full_content_included"] is False, "46 no full")
    assert_true(report["phase46_blocked_llm_output_review"]["ready_for_retry"] is False, "46 no retry")
    assert_true(report["phase46_blocked_llm_output_review"]["retry_requires_new_manual_gate"] is True, "46 new gate")
    assert_true(report["phase46_llm_retry_policy"]["automatic_retry_allowed"] is False, "46 no auto retry")
...
def test_phase36e_actual_llm_draft_call_closeout_summary() -> None:
    ...
    assert_true(closeout["llm_response_packet_created"] is False, "Packet not created")
    assert_true(closeout["output_safety_allowed"] is False, "Output not allowed")
    assert_true(closeout["output_safety_blocked"] is True, "Output blocked")
